# https://levelup.gitconnected.com/using-nix-flakes-with-bazel-8189719ea799
{
  ...
  inputs.polysemy-src.url = "github:polysemy-research/polysemy/v1.7.1.0";
  inputs.servant = {
  ...
  outputs = all@{ self, nixpkgs, flake-utils, easy-hls-src, polysemy-src
    , servant, servant-openapi3, universum, ... }:
    let
      ...
      let
        overlays = {
          haskell = import ./api/deps.nix {
            polysemyPkgs = polysemy-src.packages.${system};
            servantSrcs = servant;
